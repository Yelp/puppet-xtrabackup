...
prune_xtrabackups() {
  # Wait 24 hours after taking a successful backup before pruning
  local wait_to_prune=24
  ...
  backup_success_file="$(find ${backup_root}/${cluster} -maxdepth 1 -name "*xbstream.gz.success" | sort | tail -n 1 | head -n 1)"
  ...
  tmpwatch -v -m "$tmpwatch_threshold" "${backup_root}/${cluster}"
}
